...
export default function SummaryCards({ cards = [] }) {
    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {cards.map((card, idx) => (
                <div key={idx} className="p-4 bg-white rounded-xl border border-slate-200 shadow-sm flex items-center justify-between transition-all hover:shadow-md">
                    <div>
                        <p className={`text-xs font-bold uppercase tracking-wider ${card.textColor || 'text-slate-500'}`}>
                            {card.title}
                        </p>
